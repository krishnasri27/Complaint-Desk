...
// FIX 1: Fix navigation after verification
const oldRegNav = `        if (data.user && data.user.role === 'admin') {
          fetchComplaints();
...
const newRegNav = `        if (data.user && data.user.role === 'admin') {
          fetchComplaints();
          navigateTo('adminDashboard');
        } else if (data.user) {
...
code = code.replace(oldRegNav, newRegNav);

// FIX 2: Re-insert User Dashboard View
const insertMarker = `{/* ── COMPLAINT FORM ── */}`;
...
if (!code.includes("view === 'userDashboard'")) {
    code = code.replace(insertMarker, userDashboardJSX + insertMarker);
}

fs.writeFileSync(appPath, code, 'utf8');
console.log('Fixed User Dashboard rendering and registration navigation.');
