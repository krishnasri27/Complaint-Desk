...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('COMPLAINT FORM')) {
    targetLine = i;
    break;
  }
}
...
if (targetLine !== -1) {
  lines.splice(targetLine, 0, userDashboardJSX);
  fs.writeFileSync(appPath, lines.join('\n'), 'utf8');
  console.log('Inserted userDashboardJSX at line ' + (targetLine + 1));
} else {
  console.error('Could not find COMPLAINT FORM marker');
}
